...
function checkDataEmpCode(frompage) {
    ...
    if (checkDataInputRequest(arrIdInput)) {
        ...
        let empEmail = byId('empEmail').value
        ...
        console.log(data)
        connectApi('auth/loginwithempcode', data, 'login-page', function(output) {
            console.log(output)
            if (output.status === 200) {
                if (DataType == 'empcode') {
                    ...
                    label.innerHTML = `ระบุ OTP ที่ได้รับทาง Email`
                    ...
                    byId('empEmail').setAttribute('disabled', 'true')
                    byId('empEmail').classList.remove('is-valid')
